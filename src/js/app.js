...
var click = function(ev) {
    ...
    fetch('/data/clear/upload.json')
        ...
        .then(function(res) {
            ...
            return fetch('/res.arm.request.grp//statement/upload', {
                method: 'POST',
                body: data,
                headers: {
        ...
        .then((resp) => resp.json())
        .then(function(gettedData) {

            debugger;

            var contentToSign = gettedData.contentToSign;

            // получаем сертификат
            return SignatureService.getCertificates()
                .then(function(data) {
                    var _c = data[0];
                    var promisesList = [gettedData]; // данные с доками полученными с сервера в первом элементе

                    for (var k in contentToSign) {
                        if (contentToSign.hasOwnProperty(k)) {
                            // подписываем данные 
                            promisesList.push(SignatureService.doSign(_c, contentToSign[k]));
                        }
                    }
                ...
                .then(function(res) {
                    ...
                    var doc = res.shift(); // данные с доками полученными с сервера

                    var obj = {
                        packageGuid: doc.packageGuid,
                        statementGuid: doc.statementGuid,
                        signs: {}
                    };

                    for (var k in doc.contentToSign) {
                        if (doc.contentToSign.hasOwnProperty(k)) {
                            obj.signs[k] = res.shift();
                        }
                    }
        ...
        .then(function(data) {
            ...
            console.log('/res.arm.request.grp//statement/uploadSign');

            return fetch('/res.arm.request.grp//statement/uploadSign', {
                method: 'POST',
                body: data,
                headers: {
                    'Accept': 'application/json, text/plain, */*',
...
document.getElementById('sig-btn-2').addEventListener('click', click2, false);
document.getElementById('sig-btn-3').addEventListener('click', click3, false);
...
function click2() {
    ...
    SignatureService.getCertificates().then(data => {
            var _c = data[0];
